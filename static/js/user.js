  ...
  canvas.toBlob(async function(blob) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert('Error: ' + data.error);  // Display the error from backend (e.g., blurry image)
      }
    } catch (error) {
...
document.getElementById('uploadForm').addEventListener('submit', async function(event) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      alert('Error: ' + data.error); // Display error message from backend
    }
  } catch (error) {
...
document.getElementById("logoutLink").addEventListener("click", () => {
  // Remove the auth token from localStorage (or sessionStorage, depending on where it is stored)
  localStorage.removeItem("authToken");

  // Redirect the user to the login page
  window.location.href = "home.html"; // Make sure the URL points to your login page
});
